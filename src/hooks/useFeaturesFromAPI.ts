// Hook to fetch translated feature data from backend API with fallback to local data
...
export function useFeaturesFromAPI(): UseFeaturesResponse {
  ...
  useEffect(() => {
    const fetchFeatures = async () => {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/api/features?lang=${language}`, {
          // Add timeout to detect if backend is available
        ...
        console.log('API not available, using local fallback:', err);
        // Use local fallback data
        setFeatures(getLocalFeatures(language));
        setIsFromAPI(false);
        // Don't set error for fallback - it's expected behavior
      } finally {
        setLoading(false);
      }
    };
  ...
  return { features, loading, error, isFromAPI };
}
