...
export default function SynastryViewPage() {
  ...
  useEffect(() => {
    ...
    const hasSeenTour = localStorage.getItem('synastry-tour');
    if (!hasSeenTour) {
      const timer = setTimeout(() => setIsTourActive(true), 2000); // Slightly longer for synastry as it loads more data
      return () => {
          ...
          {loading ? (
            ...
            <>
              ...
              <VibesLinkContainer $isLight={isLight}>
                <button onClick={() => window.location.href = `/synastry-board`}>
                  Khám Phá Aesthetic Board Song Hành
                </button>
              </VibesLinkContainer>
