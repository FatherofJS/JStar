...
export default function BoardPage() {
  ...
  const state = location.state as BoardState | null;
  const chartData = state?.chartData || null;

  // Also try sessionStorage as fallback
  const cachedData = (() => {
    if (chartData) return chartData;
    try {
      const raw = sessionStorage.getItem('jstar_chart_cache');
      if (raw) return JSON.parse(raw) as ChartData;
    } catch { /* ignore */ }
    return null;
  })();
  ...
  return (
    <Layout>
      ...
      <PageWrapper $isLight={isLight}>
        {cachedData ? (
          <PersonalizedBoard data={cachedData} isLight={isLight} onBack={() => navigate(-1)} />
        ) : (
          <>
            ...
            <h2>Chưa có dữ liệu bản đồ sao</h2>
            <p>
              Bạn cần tạo bản đồ sao trước để xem Aesthetic Board cá nhân. Mỗi hành tinh sẽ tạo ra một vibe khác nhau!
            </p>
